...
def load_checkpoint(config, model, optimizer, lr_scheduler, loss_scaler, logger):
    ...
    if (
        ...
        if "max_plcc_a" in checkpoint:
            max_plcc_a = checkpoint["max_plcc_a"]
        if "max_plcc_b" in checkpoint:
            max_plcc_b = checkpoint["max_plcc_b"]
        epoched = checkpoint["epoch"]
    del checkpoint
    torch.cuda.empty_cache()
    return max_plcc_a, max_plcc_b, epoched
...
def save_checkpoint(
    config, epoch, model, max_plcc_a, max_plcc_b, optimizer, lr_scheduler, loss_scaler, logger, i
):
    base_path = os.path.join("/home/pws/backup/test", str(i), config.TAG)
    ...
    save_state = {
        ...
        "max_plcc_a": max_plcc_a,
        "max_plcc_b": max_plcc_b,
        "scaler": loss_scaler.state_dict(),
        "epoch": epoch,
        "config": config,
    }
    save_path = os.path.join(base_path, f"a_ckpt_epoch_{epoch}.pth")
...
def save_checkpoint1(
    config, epoch, model, slice_num, max_plcc_a, max_plcc_b, optimizer, lr_scheduler, loss_scaler, logger, i
):
    base_path = os.path.join("/home/pws/backup/test", str(i), config.TAG)
    if not os.path.exists(base_path):
        os.makedirs(base_path)
    ...
    save_path = os.path.join(base_path, f"b_{slice_num}_ckpt_epoch_{epoch}.pth")
    logger.info(f"{save_path} saving......")
    torch.save(save_state, save_path)
    logger.info(f"{save_path} saved !!!")
