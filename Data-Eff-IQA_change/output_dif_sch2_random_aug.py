...
from IQA import IQA_build_loader_dif, IQA_build_loader2_dif, IQA_build_loader3_dif_random, IQA_build_loader5_dif_random
...
def main(config):
    ...
    # dataset A 训练集以及测试集
    dataset_train_A, dataset_val_A, data_loader_train_A, data_loader_train_val_A, data_loader_val_A, mixup_fn_A = IQA_build_loader_dif(config, 1)
    logger.info(f"{len(data_loader_train_val_A)}")
    # dataset B 训练集以及测试集
    dataset_train_B, dataset_val_B, data_loader_train_B, data_loader_train_val_B, data_loader_val_B, mixup_fn_B = IQA_build_loader2_dif(config, 0)
    ...
    examplar_total = get_A_examplar(examplar_total, dataset_train_A, data_loader_train_ex1, s_model, config.DATA.EXAMPLAR_STEP)
    ...
    for slice_num in range(config.DATA.SLICE_NUMBER):
        ...
        dataset_train1, data_loader_train1, mixup_fn = IQA_build_loader3_dif_random(config, train_index, examplar_total, 0)
        if dist.get_rank() == 0:
        ...
        for epoch in range(config.TRAIN.START_EPOCH_STU1, config.TRAIN.EPOCHS_STU1):
            ...
            srcc_1_b, plcc_1_b, loss_1_b = train_one_epoch2(config, s_model, t_model, criterion, data_loader_train1, slice_num,
        ...
        if slice_num < config.DATA.SLICE_NUMBER-1:
            # for select examplar
            dataset_train2 = IQA_build_loader5_dif_random(config, train_index, train_set, 0)
            ...
            examplar_total = get_B_examplar(examplar_total, dataset_train2, data_loader_train2_ex2, s_model, t_model, config.DATA.EXAMPLAR_STEP1)
            if dist.get_rank() == 0:
                logger.info(f"len(examplar_total):{len(examplar_total)}")
...
def train_one_epoch2(
    ...
    for idx, (samples, sample1, target, data_num) in enumerate(data_loader):
        samples = samples.cuda(non_blocking=True)
        sample1 = sample1.cuda(non_blocking=True)

        with torch.no_grad():
            targets = t_model(sample1)
...
def get_A_examplar(examplar, dataset_train, data_loader_train_cv, s_model, examplar_step):
    s_model.eval()
    top_k = math.ceil(math.ceil(len(dataset_train) / examplar_step))
    print(top_k)
    point_array = []
    for idx, (image, _, target, _) in enumerate(data_loader_train_cv):
        with torch.no_grad():
...
def get_B_examplar(examplar, dataset_train, data_loader_train_cv, s_model, t_model, examplar_step):
    ...
    print(top_k)
    point_array = []
    for idx, (image, image1, _, _) in enumerate(data_loader_train_cv):
        with torch.no_grad():
            image = image.cuda(non_blocking=True)
...
if __name__ == "__main__":
    ...
    for i in range(num):
        ...
        plcc1_notrain_average = '%.4f' % (plcc1_notrain_total / (i+1))
        srcc1_notrain_average = '%.4f' % (srcc1_notrain_total / (i+1))
        plcc2_notrain_average = '%.4f' % (plcc2_notrain_total / (i+1))
        srcc2_notrain_average = '%.4f' % (srcc2_notrain_total / (i+1))
        plcc1_train_average = '%.4f' % (plcc1_train_total / (i+1))
        srcc1_train_average = '%.4f' % (srcc1_train_total / (i+1))
        plcc2_train_average = '%.4f' % (plcc2_train_total / (i+1))
        srcc2_train_average = '%.4f' % (srcc2_train_total / (i+1))
