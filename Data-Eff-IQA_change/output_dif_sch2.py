...
from IQA import (
    IQA_build_loader_dif, IQA_build_loader2_dif,
    IQA_build_loader3_dif, IQA_build_loader5_dif,
    IQA_build_loader3_dif_random, IQA_build_loader5_dif_random,
)
...
def parse_option():
    ...
    # os.environ["WANDB_API_KEY"] = "bf65a8ca935bfbf21238c6b60a13cc94241af82e"
...
def main(config):
    ...
    for slice_num in range(config.DATA.SLICE_NUMBER):
        ...
        for epoch in range(config.TRAIN.START_EPOCH_STU1, config.TRAIN.EPOCHS_STU1):
            ...
            if dist.get_rank() == 0 and (
                    epoch % config.SAVE_FREQ == 0 or epoch == (config.TRAIN.EPOCHS_STU1 - 1)
            ):
                save_checkpoint1(
                    config,
                    epoch,
...
if __name__ == "__main__":
    ...
    for i in range(num):
        ...
        # if i == num-1:
        if i % 2 == 1 and i > 0:
            plcc_srcc1.sort(key=lambda x: (x[4], x[5]))
            plcc_srcc2.sort(key=lambda x: (x[6], x[7]))
            plcc_srcc3.sort(key=lambda x: (x[4] - x[0], x[5] - x[1]))
