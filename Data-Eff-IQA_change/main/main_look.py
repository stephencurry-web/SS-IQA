...
def main(config):
    ...
    if dist.get_rank() == 0:
        ...
        # wandb_runner.log({"Validating PLCC": 0.0, "Validating SRCC": 0.0, "Epoch": 0})
    else:
        wandb_runner = None
    ...
    #         config, s_model_without_ddp, optimizer, lr_scheduler, loss_scaler1, logger
    #     )
    #     srcc, plcc, loss = validate(
    #         config, data_loader_val, s_model, epochs, 0, len(dataset_val)
    #     )
    #     logger.info(
    ...
    #     srcc, plcc, loss = validate(config, data_loader_val, s_model, 0, 0)
    #     logger.info(
    #         f"SRCC and PLCC of the network on the {len(dataset_val)} test images: {srcc:.6f}, {plcc:.6f}"
    #     )
    #
    ...
    for epoch in range(config.TRAIN.START_EPOCH_STU, config.TRAIN.EPOCHS_STU):
        ...
        train_one_epoch(config, s_model, criterion, data_loader_train, data_loader_val, len(dataset_val),
                        data_loader_val_total, len(dataset_val_total),
        ...
        logger.info(
            f"SRCC and PLCC of the network on the {len(dataset_val)} test images: {srcc_1_a:.6f}, {plcc_1_a:.6f}"
        )
        ...
        logger.info(
            f"SRCC and PLCC of the network on the {len(dataset_val_total)} test images: {srcc_1_b:.6f}, {plcc_1_b:.6f}"
        )
    ...
    for slice_num in range(config.DATA.SLICE_NUMBER):
        ...
        len1 = len(config.SET1.TRAIN_INDEX)
        len2 = len(config.SET1.TEST_INDEX)
        if slice_num == 0:
            train_index = config.SET1.TRAIN_INDEX[0:len1 // 2]
            val_index = config.SET1.TEST_INDEX[0:len2 // 2]
        else:
            for i, index in enumerate(config.SET1.TRAIN_INDEX[len1 // 2:]):
                if (i % (config.DATA.SLICE_NUMBER - 1) == (slice_num - 1)):
                    train_index.append(index)
            logger.info(f"train_index:{train_index}")
            for i, index in enumerate(config.SET1.TEST_INDEX[len2 // 2:]):
                if (i % (config.DATA.SLICE_NUMBER - 1) == (slice_num - 1)):
        ...
        for epoch in range(config.TRAIN.START_EPOCH_STU1, config.TRAIN.EPOCHS_STU1):
            ...
            train_one_epoch2(config, s_model, t_model, criterion, data_loader_train1, data_loader_val, len(dataset_val),
                             data_loader_val_total, len(dataset_val_total), slice_num,
                            optimizer2, epoch, mixup_fn, lr_scheduler1, loss_scaler2, writer, wandb_runner)


            # if dist.get_rank() == 0 and (
            #         epoch % config.SAVE_FREQ == 0 or epoch == (config.TRAIN.EPOCHS1 - 1)
            # ):
            #     save_checkpoint1(config, epoch, s_model_without_ddp, slice_num, max_plcc_2_b, optimizer2,
            #                     lr_scheduler, loss_scaler2, logger)
            # if slice_num >= 0:
...
def train_one_epoch2(
    ...
    for idx, (samples, target, data_num) in enumerate(data_loader):
        ...
        if idx % config.PRINT_FREQ == 0:
            ...
            srcc_1_a, plcc_1_a, loss_1_a = validate2(config, data_loader_val1, s_model, epoch, slice_num, 1,
                                                    dataset_val1_len, tensorboard, wandb_runner)

            if dist.get_rank() == 0:
                wandb_runner.log(
                    {f"slice{slice_num} look Validating srcc1": srcc_1_a, f"slice{slice_num} look Validating plcc1": plcc_1_a,
                     f"slice{slice_num} look Validating loss1": loss_1_a, "Epoch": epoch}
                )

            srcc_1_b, plcc_1_b, loss_1_b = validate3(config, data_loader_val2, s_model, epoch, slice_num, 2,
                                                     dataset_val2_len, tensorboard, wandb_runner)

            if dist.get_rank() == 0:
                wandb_runner.log(
                    {f"slice{slice_num} look Validating srcc2": srcc_1_b, f"slice{slice_num} look Validating plcc2": plcc_1_b,
                     f"slice{slice_num} look Validating loss2": loss_1_b, "Epoch": epoch}
                )
...
def validate2(
    ...
    logger.info(f"slice{slice_num} validate{validate_num} * SRCC@ {test_srcc:.6f} PLCC@ {test_plcc:.6f}")
...
def train_one_epoch(
    ...
    for idx, (samples, targets, _) in enumerate(data_loader):
        ...
        if idx % config.PRINT_FREQ == 0:
            ...
            srcc_1_a, plcc_1_a, loss_1_a = validate(config, data_loader_val1, s_model, epoch, 1,
                                                    dataset_val1_len, tensorboard, wandb_runner)

            if dist.get_rank() == 0:
                wandb_runner.log(
                    {"dataset_a look Validating srcc1": srcc_1_a, "dataset_a look Validating plcc1": plcc_1_a,
                     "dataset_a look Validating loss1": loss_1_a, "Epoch": epoch}
                )

            srcc_1_b, plcc_1_b, loss_1_b = validate1(config, data_loader_val2, s_model, epoch, 2,
                                                    dataset_val2_len, tensorboard, wandb_runner)

            if dist.get_rank() == 0:
                wandb_runner.log(
                    {"dataset_a look Validating srcc2": srcc_1_b, "dataset_a look Validating plcc2": plcc_1_b,
                     "dataset_a look Validating loss2": loss_1_b, "Epoch": epoch}
                )
