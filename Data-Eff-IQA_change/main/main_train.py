...
from IQA import IQA_build_loader, IQA_build_loader2, IQA_build_loader3, SubsetRandomSampler
from config import get_config1
...
def parse_option():
    ...
    config = get_config1(args, local_rank)
    return args, config
...
def main(config):
    ...
    # image, target, num = dataset_train[0]
    dataset_train_total, dataset_val_total, data_loader_train_total, data_loader_val_total, mixup_fn = IQA_build_loader2(config, 0)
    # examplar_total = get_examplar(examplar_total, dataset_train)
    # examplar_total = get_examplar1(examplar_total, data_loader_train_cv, s_model, criterion)
    ...
    srcc_wo_train, plcc_wo_train, loss_wo_train = validate(config, data_loader_val_total, s_model, epoch,
                                                           len(dataset_val_total), tensorboard=writer,
                                                           wandb_runner=wandb_runner)
    if dist.get_rank() == 0:
        wandb_runner.log(
            {"Validating SRCC_wo_train": srcc_wo_train, "Validating PLCC_wo_train": plcc_wo_train}
        )
    logger.info(
        f"SRCC and PLCC of the network on the {len(dataset_val_total)} test images: {srcc_wo_train:.6f}, {plcc_wo_train:.6f}"
    )
    ...
    if config.TRAIN.ACCUMULATION_STEPS > 1:
        lr_scheduler1 = build_scheduler2(
            config, optimizer, len(data_loader_train_total) // config.TRAIN.ACCUMULATION_STEPS
        )
    else:
        lr_scheduler1 = build_scheduler2(config, optimizer, len(data_loader_train_total))
    ...
    for epoch in range(config.TRAIN.START_EPOCH_STU1, config.TRAIN.EPOCHS_STU1):
        ...
        data_loader_train_total.sampler.set_epoch(epoch)

        train_one_epoch2(config, s_model, t_model, criterion, data_loader_train_total, 1,
        ...
        if dist.get_rank() == 0 and (
            ...
            save_checkpoint1(config, epoch, s_model_without_ddp, 1, max_plcc2, optimizer,
                            lr_scheduler, loss_scaler, logger)

        srcc_cv1, plcc_cv1, loss1 = validate2(config, data_loader_val, s_model, epoch, 1,
                                              len(dataset_val), tensorboard=writer, wandb_runner=wandb_runner)
        srcc_cv2, plcc_cv2, loss2 = validate2(config, data_loader_val_total, s_model, epoch, 1,
                                           len(dataset_val_total), tensorboard=writer, wandb_runner=wandb_runner)

        if config.TENSORBOARD == True:
            writer.add_scalars(
                f"dataValidate Metrics",
        ...
        if dist.get_rank() == 0:
            wandb_runner.log(
                {f"dataValidating SRCC1": srcc_cv1,
                 f"dataValidating PLCC1": plcc_cv1,
                 f"dataValidating SRCC2": srcc_cv2,
                 f"dataValidating PLCC2": plcc_cv2,
                 f"dataEpoch": epoch + 1}
            )
        logger.info(
            # f"data{slice_num}SRCC and PLCC of the network on the {len(dataset_val)} test images: {srcc_cv:.6f}, {plcc_cv:.6f}"
            f"data SRCC1 and PLCC1 of the network on the {len(dataset_val)} test images: {srcc_cv1:.6f}, {plcc_cv1:.6f}; "
        ...
        if plcc_cv1 >= max_plcc1:
            ...
            max_srcc1 = 0
        logger.info(f"dataMax PLCC1: {max_plcc1:.6f} Max SRCC1: {max_srcc1:.6f}")
        if dist.get_rank() == 0:
        ...
        if plcc_cv2 >= max_plcc2:
            ...
            max_srcc2 = 0
        logger.info(f"data Max PLCC2: {max_plcc2:.6f} Max SRCC2: {max_srcc2:.6f}")
        if dist.get_rank() == 0:
    ...
    total_time = time.time() - start_time
    total_time_str = str(datetime.timedelta(seconds=int(total_time)))
    logger.info("data Training time {}".format(total_time_str))
    if dist.get_rank() == 0:
        wandb_runner.alert(
            title="Run Finished",
