...
from IQA import IQA_build_loader,  IQA_build_loader2, IQA_build_loader3
from config import get_config
from logger import create_logger
from lr_scheduler import build_scheduler, build_scheduler1, build_scheduler2
...
def main(config):
    ...
    #     return
    # examplar_img_total = []
    # examplar_label_total = []
    examplar_total = []
    # val_total = []

    dataset_train, dataset_val, data_loader_train, data_loader_val, mixup_fn = IQA_build_loader(config)
    dataset_train_total, dataset_val_total, data_loader_train_total, data_loader_val_total, mixup_fn = IQA_build_loader2(config)
    examplar_total = get_examplar(examplar_total, dataset_train)
    # val_total = dataset_val
    if dist.get_rank() == 0:
        logger.info(f"len(examplar_total):{len(examplar_total)}")
        # logger.info(f"len(val_total):{len(val_total)}")
    ...
    for epoch in range(config.TRAIN.START_EPOCH_STU, config.TRAIN.EPOCHS_STU):
        ...
        # examplar_img, examplar_label = train_one_epoch(config, s_model, criterion, data_loader_train,
        #  optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler, writer, wandb_runner)
        ...
        if dist.get_rank() == 0 and (
            ...
            save_checkpoint(config, epoch, s_model_without_ddp, max_plcc, optimizer,
                lr_scheduler, loss_scaler, logger)
        srcc, plcc, loss = validate(config, data_loader_val, s_model, epoch,
            len(dataset_val), tensorboard=writer, wandb_runner=wandb_runner,)
    ...
    logger.info("first data Training time {}".format(total_time_str))
    if dist.get_rank() == 0:
        wandb_runner.alert(
            title="Run Finished",
    ...
    train_index, val_index = config.SET1.TRAIN_INDEX, config.SET1.TEST_INDEX
    ...
    #     = IQA_build_loader1(config, train_index, val_index, examplar_img_total, examplar_label_total)
    dataset_train1, dataset_val1, data_loader_train1, data_loader_val1, mixup_fn \
        = IQA_build_loader3(config, train_index, val_index, examplar_total)
    examplar_total = get_examplar(examplar_total, dataset_train1)
    # val_total = get_val(dataset_val1)

    if config.TRAIN.ACCUMULATION_STEPS > 1:
        lr_scheduler1 = build_scheduler2(
            config, optimizer, len(data_loader_train1) // config.TRAIN.ACCUMULATION_STEPS
        )
    else:
        lr_scheduler1 = build_scheduler2(config, optimizer, len(data_loader_train1))
    ...
    # begin_count_train = slice_num * slice_train_size
    # begin_count_val = slice_num * slice_val_size
    # if slice_num == config.DATA.SLICE_NUMBER:
    #     end_count_train = incre_train_size + 1
    #     end_count_val = incre_val_size + 1
    # else:
    #     end_count_train = (slice_num + 1) * slice_train_size
    #     end_count_val = (slice_num + 1) * slice_val_size
    max_plcc1 = 0.0
    max_plcc2 = 0.0
    max_srcc1 = 0.0
    max_srcc2 = 0.0
    for epoch in range(config.TRAIN.START_EPOCH_STU1, config.TRAIN.EPOCHS_STU1):
        # print("dataset_train and val slice:{},{},{},{}".format(begin_count_train, end_count_train, begin_count_val, end_count_val))
        # data_loader_train1, data_loader_val1 = IQA_build_loader1(config,
        #     dataset_train1[begin_count_train:end_count_train, :, :], dataset_val1[begin_count_val:end_count_val, :, :])
        data_loader_train1.sampler.set_epoch(epoch)

        train_one_epoch1(config, s_model, t_model, criterion, data_loader_train1,
                        optimizer, epoch, mixup_fn, lr_scheduler1, loss_scaler, writer, wandb_runner)


        if dist.get_rank() == 0 and (
                epoch % config.SAVE_FREQ == 0 or epoch == (config.TRAIN.EPOCHS - 1)
        ):
            save_checkpoint2(config, epoch, s_model_without_ddp, max_plcc2, optimizer,
                            lr_scheduler, loss_scaler, logger)

        # srcc_cv, plcc_cv, loss = validate2(config, data_loader_val_total, s_model, epoch, slice_num,
        #                                    len(dataset_val_total), tensorboard=writer, wandb_runner=wandb_runner)
        # if(slice_num == config.DATA.SLICE_NUMBER-1):
            srcc_cv1, plcc_cv1, loss1 = validate1(config, data_loader_val, s_model, epoch,
                                                  len(dataset_val), tensorboard=writer, wandb_runner=wandb_runner)
            srcc_cv2, plcc_cv2, loss2 = validate1(config, data_loader_val_total, s_model, epoch,
                                               len(dataset_val_total), tensorboard=writer, wandb_runner=wandb_runner)

            if config.TENSORBOARD == True:
                writer.add_scalars(
                    f"data2Validate Metrics",
                    {"SRCC1": srcc_cv1, "PLCC1": plcc_cv1, "SRCC2": srcc_cv2, "PLCC2": plcc_cv2},
                    epoch,
                )

            if dist.get_rank() == 0:
                wandb_runner.log(
                    {f"data2Validating SRCC1": srcc_cv1,
                     f"data2Validating PLCC1": plcc_cv1,
                     f"data2Validating SRCC2": srcc_cv2,
                     f"data2Validating PLCC2": plcc_cv2,
                     f"data2Epoch": epoch + 1}
                )
            logger.info(
                # f"data{slice_num}SRCC and PLCC of the network on the {len(dataset_val)} test images: {srcc_cv:.6f}, {plcc_cv:.6f}"
                f"data2 SRCC1 and PLCC1 of the network on the {len(dataset_val)} test images: {srcc_cv1:.6f}, {plcc_cv1:.6f}; "
                f"SRCC2 and PLCC2 of the network on the {len(dataset_val_total)} test images: {srcc_cv2:.6f}, {plcc_cv2:.6f}"

            )
            if plcc_cv1 >= max_plcc1:
                max_plcc1 = max(max_plcc1, plcc_cv1)
                max_srcc1 = srcc_cv1
            elif plcc_cv1 < 0:
                max_srcc1 = 0
            logger.info(f"data2Max PLCC1: {max_plcc1:.6f} Max SRCC1: {max_srcc1:.6f}")
            if dist.get_rank() == 0:
                wandb_runner.summary["Best PLCC1"], wandb_runner.summary["Best SRCC1"] = (
                    max_plcc1,
                    max_srcc1,
                )

            if plcc_cv2 >= max_plcc2:
                max_plcc2 = max(max_plcc2, plcc_cv2)
                max_srcc2 = srcc_cv2
            elif plcc_cv2 < 0:
                max_srcc2 = 0
            logger.info(f"data2 Max PLCC2: {max_plcc2:.6f} Max SRCC2: {max_srcc2:.6f}")
            if dist.get_rank() == 0:
                wandb_runner.summary["Best PLCC2"], wandb_runner.summary["Best SRCC2"] = (
                    max_plcc2,
                    max_srcc2,
                )
        # srcc_total, plcc_total, loss = validate2(config, data_loader_val_total, s_model, epoch, slice_num,
        #                                          len(dataset_val_total), tensorboard=writer, wandb_runner=wandb_runner)
        # logger.info(f"data{slice_num}Total PLCC: {plcc_total:.6f} Total SRCC: {srcc_total:.6f}")
        total_time = time.time() - start_time
        total_time_str = str(datetime.timedelta(seconds=int(total_time)))
        logger.info("{} data Training time {}".format(2, total_time_str))
        if dist.get_rank() == 0:
            wandb_runner.alert(
                title="Run Finished",
                text=f"Max PLCC1: {max_plcc1:.6f} Max SRCC1: {max_srcc1:.6f} Max PLCC2: {max_plcc2:.6f} Max SRCC2: {max_srcc2:.6f} Training time: {total_time_str}",
            )
...
def get_examplar(examplar, dataset_train):
    # examplar_num = len(dataset_train)/config.DATA.EXAMPLAR_STEP
    # logger.info(f"examplar_num:{examplar_num}")
    # select_num = random_int_list(0, len(dataset_train), examplar_num)
    # logger.info(f"len(dataset_train):{len(dataset_train)}")
    # logger.info(f"select_num:{select_num}")
    # for idx in select_num:
    #     examplar.append(dataset_train.samples[idx])
...
def get_val(dataset_val):
    val_total = []
    for idx, item in enumerate(dataset_val):
        val_total.append(item)
    return val_total
...
def train_one_epoch2(
    ...
    print("num_step:{}".format(num_steps))
    ...
    for idx, (samples, _) in enumerate(data_loader):
        ...
        if idx % config.PRINT_FREQ == 0:
            if config.TENSORBOARD == True:
                tensorboard.add_scalars(
                    f"data{slice_num}Training Loss",
                    {"Loss": loss_meter.val},
            ...
            if wandb_runner:
                wandb_runner.log(
                    {
                        f"data{slice_num}Training Loss": loss_meter.val,
                        f"data{slice_num}Learning Rate": lr,
                        f"data{slice_num}Batch": epoch * len(data_loader) + idx,
                    },
                )
            memory_used = torch.cuda.max_memory_allocated() / (1024.0 * 1024.0)
            etas = batch_time.avg * (num_steps - idx)
            logger.info(
                f"data{slice_num}Train: [{epoch+1}/{config.TRAIN.EPOCHS_STU1}][{idx}/{num_steps}]\t"
                f"data{slice_num}eta {datetime.timedelta(seconds=int(etas))} lr {lr:.6f}\t wd {wd:.4f}\t"
                f"data{slice_num}time {batch_time.val:.4f} ({batch_time.avg:.4f})\t"
                f"data{slice_num}loss {loss_meter.val:.4f} ({loss_meter.avg:.4f})\t"
                ...
                f"data{slice_num}grad_norm {norm_meter.val:.4f} ({norm_meter.avg:.4f})\t"
                f"data{slice_num}loss_scale {scaler_meter.val:.4f} ({scaler_meter.avg:.4f})\t"
                f"data{slice_num}mem {memory_used:.0f}MB"
            )
    epoch_time = time.time() - start
    train_srcc, _ = stats.spearmanr(pred_scores, gt_scores)
    if config.TENSORBOARD == True:
        tensorboard.add_scalar(
            f"data{slice_num}Training SRCC",
            train_srcc,
            epoch,
        )
    logger.info(
        f"data{slice_num}EPOCH {epoch} training takes {datetime.timedelta(seconds=int(epoch_time))}"
    )
    logger.info(f"data{slice_num}EPOCH {epoch} training SRCC: {train_srcc}")


@torch.no_grad()
def validate2(
    config, data_loader, s_model, epoch, slice_num, val_len, tensorboard=None, wandb_runner=None
):
    criterion = torch.nn.SmoothL1Loss()
    s_model.eval()
    batch_time = AverageMeter()
    ...
    for idx, (images, target) in enumerate(data_loader):
        ...
        if idx % config.PRINT_FREQ == 0:
            if config.TENSORBOARD == True and tensorboard != None:
                tensorboard.add_scalar(
                    f"data{slice_num}Validating Loss",
                    loss_meter.val,
                    epoch * len(data_loader) + idx,
                )
            if wandb_runner:
                wandb_runner.log(
                    {
                        f"data{slice_num}Validating Loss": loss_meter.val,
                        f"data{slice_num}Validate Batch": epoch * len(data_loader) + idx,
                    }
                )
            memory_used = torch.cuda.max_memory_allocated() / (1024.0 * 1024.0)
            logger.info(
                f"data{slice_num}Test: [{idx}/{len(data_loader)}]\t"
                f"data{slice_num}Time {batch_time.val:.3f} ({batch_time.avg:.3f})\t"
                f"data{slice_num}Loss {loss_meter.val:.4f} ({loss_meter.avg:.4f})\t"
                f"data{slice_num}Mem {memory_used:.0f}MB"
            )
    ...
    # print("finish3")
    logger.info(f"data{slice_num} * SRCC@ {test_srcc:.6f} PLCC@ {test_plcc:.6f}")
...
def train_one_epoch(
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        # if idx % config.DATA.EXAMPLAR_STEP == 0 & epoch == 0:
        #     examplar_img.append(samples)
        #     print(targets)
        #     examplar_label.append(targets)
