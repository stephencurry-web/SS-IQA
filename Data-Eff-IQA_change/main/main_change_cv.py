...
def main(config):
    ...
    if os.path.exists(os.path.join(config.OUTPUT, 'features.npy')):
    ...
    for slice_num in range(config.DATA.SLICE_NUMBER):
        ...
        if config.TRAIN.ACCUMULATION_STEPS > 1:
            ...
            lr_scheduler_s1 = build_scheduler2(
            ...
            lr_scheduler_s1 = build_scheduler2(config, optimizer_s1, len(data_loader_train1))
        ...
        for epoch in range(config.TRAIN.START_EPOCH_STU1, config.TRAIN.EPOCHS_STU1):
            ...
            train_one_epoch2(config, s_model, s1_model, t_model, criterion, data_loader_train1, slice_num,
                            optimizer2, optimizer_s1, epoch, mixup_fn, lr_scheduler1, lr_scheduler_s1, loss_scaler2, loss_scaler3, writer, wandb_runner)
