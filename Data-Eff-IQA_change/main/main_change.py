...
from IQA import IQA_build_loader, IQA_build_loader2, IQA_build_loader3, IQA_build_loader5, IQA_build_loader_kmeans
from config import get_config
from logger import create_logger
from lr_scheduler import build_scheduler, build_scheduler1, build_scheduler2, build_scheduler3
from models import build_model_pws
...
def main(config):
    ...
    s_model = build_model_pws(config)
    s1_model = build_model_pws(config)
    # logger.info(str(s_model))

    t_model = build_model_pws(config)
    ...
    dataset_kmeans, dataset_loader_kmeans = IQA_build_loader_kmeans(config, 1)
    ...
    optimizer_s1 = build_optimizer(config, s1_model)
    ...
    s1_model = torch.nn.parallel.DistributedDataParallel(
        s1_model,
        device_ids=[config.LOCAL_RANK],
        broadcast_buffers=False,
        # find_unused_parameters=True,
    ...
    if os.path.exists(os.path.join(config.OUTPUT, 'features1.npy')):
        features = np.load(os.path.join(config.OUTPUT, 'features.npy'))
        ...
        with torch.no_grad():
            ...
            N = len(dataset_loader_kmeans)  # for processing image pairs
            print('extracting fc features for kmeans')
            for i, (samples, _, _) in enumerate(dataset_loader_kmeans):
                samples = samples.cuda(non_blocking=True)

                bs = samples.size(0)  # batch size
                assert bs == 1

                _, sfeat = t_model(samples)
                sfeat.squeeze(0)
                sfeat = sfeat.cpu().numpy()
                if i == 0:
                    features = np.zeros((N, sfeat.shape[1], sfeat.shape[2]), dtype='float32')
                    re_shape = sfeat.shape[1] * sfeat.shape[2]
                # print(sfeat.shape)
                # print(features.shape)
                sfeat = sfeat.astype('float32')
                features[i * bs: (i + 1) * bs, :] = sfeat
            np.save(os.path.join(config.OUTPUT, 'features.npy'), features)

        print('features saved!')


    if not os.path.exists(os.path.join(config.OUTPUT, 'cluster.pt')):
        temp_features = features
        temp_features = temp_features.reshape(len(dataset_loader_kmeans), re_shape)
        estimator = KMeans(n_clusters=config.DATA.num_cluster, n_init=20, verbose=True, max_iter=1000)
        estimator.fit(temp_features)
        cluster_name = os.path.join(config.OUTPUT, 'cluster.pt')
        torch.save(torch.from_numpy(estimator.cluster_centers_), cluster_name)

    # mean features baseline
    features = np.mean(features, axis=0)
    features = features / np.linalg.norm(features)
    mean_name = os.path.join(config.OUTPUT, 'mean_feature.pt')
    ...
    for slice_num in range(config.DATA.SLICE_NUMBER):
        ...
        # optimizer_s1 = build_optimizer1(config, s1_model)
        if config.TRAIN.ACCUMULATION_STEPS > 1:
            lr_scheduler1 = build_scheduler2(
                config, optimizer2, len(data_loader_train1) // config.TRAIN.ACCUMULATION_STEPS
            )
            lr_scheduler_s1 = build_scheduler1(
                config, optimizer_s1, len(data_loader_train1) // config.TRAIN.ACCUMULATION_STEPS
            )
        else:
            lr_scheduler1 = build_scheduler2(config, optimizer2, len(data_loader_train1))
            lr_scheduler_s1 = build_scheduler1(config, optimizer_s1, len(data_loader_train1))
        ...
        # dataset 0 val set test
        max_plcc_2_a = 0.0
        max_srcc_2_a = 0.0

        # dataset slice_num val set test
        max_plcc_2_b = 0.0
        max_srcc_2_b = 0.0

        for epoch in range(config.TRAIN.START_EPOCH_STU, config.TRAIN.EPOCHS_STU):
            data_loader_train1.sampler.set_epoch(epoch)
            train_one_epoch1(config, s1_model, t_model, criterion, data_loader_train1, slice_num,
                            optimizer_s1, epoch, mixup_fn, lr_scheduler_s1, loss_scaler3, writer, wandb_runner)

            srcc_2_a, plcc_2_a, loss_2_a = validate2(config, data_loader_val, s_model, s1_model, epoch, slice_num, 1,
                                                  len(dataset_val), writer, wandb_runner)
            srcc_2_b, plcc_2_b, loss_2_b = validate3(config, data_loader_val_total, s_model, s1_model, epoch, slice_num, 2,
                                                     len(dataset_val_total), writer, wandb_runner)


            # if config.TENSORBOARD == True:
            #     writer.add_scalars(
            #         f"slice{slice_num}Validate Metrics",
            #         {"SRCC1": srcc_2_a, "PLCC1": plcc_2_a, "SRCC2": srcc_2_b, "PLCC2": plcc_2_b},
            #         epoch,
            #     )

            if dist.get_rank() == 0:
                wandb_runner.log(
                    {f"s1_model slice{slice_num}Validating SRCC1": srcc_2_a,
                     f"s1_model slice{slice_num}Validating PLCC1": plcc_2_a,
                     f"s1_model slice{slice_num}Validating SRCC2": srcc_2_b,
                     f"s1_model slice{slice_num}Validating PLCC2": plcc_2_b,
                     f"s1_model slice{slice_num}Epoch": epoch}
                )
            logger.info(
                f"slice{slice_num} PLCC1 and SRCC1 of the network on the {len(dataset_val)} test images: {plcc_2_a:.6f}, {srcc_2_a:.6f}; "
            )
            logger.info(
                f"slice{slice_num}PLCC2 and SRCC2 of the network on the {len(dataset_val2)} test images: {plcc_2_b:.6f}, {srcc_2_b:.6f}"
            )
...
def train_one_epoch3(
    ...
    # aux_loss_fusl_meter = AverageMeter()
    norm_meter = AverageMeter()
    norm_meter_s1 = AverageMeter()
    scaler_meter = AverageMeter()
    scaler_meter_s1 = AverageMeter()
    pred_scores = []
    gt_scores = []
    start = time.time()
    end = time.time()
    ...
    for idx, (samples, target, data_num) in enumerate(data_loader):
        ...
        with torch.cuda.amp.autocast(enabled=config.AMP_ENABLE):
            # outputs, aux_score = model(samples)
            output1, feature1 = s_model(samples)
            output2, feature2 = s1_model(samples)
            output[0] = output1
            output[1] = output2
            if config.CLUSTER:
                cluster_path = os.path.join(config.OUTPUT, 'cluster.pt')
                centroids1 = torch.load(cluster_path)
                D1 = euclidean_distances(feature1.cpu().numpy(), centroids1.numpy())
                D1 = np.min(D1)
                D1 = torch.from_numpy(np.array(D1)).cuda()
                weights.append(D1)

                centroids2 = torch.load(cluster_path)
                D2 = euclidean_distances(feature2.cpu().numpy(), centroids2.numpy())
                D2 = np.min(D2)
                D2 = torch.from_numpy(np.array(D2)).cuda()
                weights.append(D2)

            else:
                meanfeat_path = os.path.join(config.OUTPUUT, 'mean_feature.pt')
                mean_feature1 = torch.load(meanfeat_path)
                mean_feature1 = mean_feature1.to(feature1)
                D1 = F.mse_loss(feature1, mean_feature1)
                weights.append(D1)

                mean_feature2 = torch.load(meanfeat_path)
                mean_feature2 = mean_feature2.to(feature2)
                D2 = F.mse_loss(feature2, mean_feature2)
                weights.append(D2)

            weights = F.softmin(weights * 32, dim=0)
            outputs = weights @ output.cuda()
        ...
        is_second_order_s1 = (
                hasattr(optimizer_s1, "is_second_order") and optimizer_s1.is_second_order
        )
        # TODO: Too see if weights help the performace
        grad_norm = loss_scaler(
            loss,
        ...
        grad_norm_s1 = loss_scaler_s1(
            loss,
            optimizer_s1,
            clip_grad=config.TRAIN.CLIP_GRAD,
            parameters=s1_model.parameters(),
            create_graph=is_second_order_s1,
            update_grad=(idx + 1) % config.TRAIN.ACCUMULATION_STEPS == 0,
        )
        if (idx + 1) % config.TRAIN.ACCUMULATION_STEPS == 0:
            optimizer.zero_grad()
            ...
            lr_scheduler_s1.step_update(
                (epoch * num_steps + idx) // config.TRAIN.ACCUMULATION_STEPS
            )
        loss_scale_value = loss_scaler.state_dict()["scale"]
        loss_scale_value_s1 = loss_scaler_s1.state_dict()["scale"]
        ...
        if grad_norm is not None:  # loss_scaler return None if not update
            norm_meter.update(grad_norm)
        if grad_norm_s1 is not None:  # loss_scaler return None if not update
            norm_meter_s1.update(grad_norm_s1)
        scaler_meter.update(loss_scale_value)
        scaler_meter_s1.update(loss_scale_value_s1)
        batch_time.update(time.time() - end)
        end = time.time()
...
@torch.no_grad()
def validate2(
    config, data_loader, s_model, s1_model, epoch, slice_num, validate_num, val_len, tensorboard=None, wandb_runner=None
):
    criterion = torch.nn.SmoothL1Loss()
    s_model.eval()
    s1_model.eval()
    ...
    for idx, (images, target, _) in enumerate(data_loader):
        ...
        # compute output
        with torch.cuda.amp.autocast(enabled=config.AMP_ENABLE):
            output1, feature1 = s_model(images)
            output2, feature2 = s1_model(images)
            output[0] = output1
            output[1] = output2
            if config.CLUSTER:
                cluster_path = os.path.join(config.OUTPUT, 'cluster.pt')
        ...
        target.unsqueeze_(dim=-1)
        temp_pred_scores.append(outputs.view(-1))
