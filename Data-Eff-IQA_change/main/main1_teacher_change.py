...
def main(config):
    ...
    # for key, value in s_model.state_dict().items():
    #     print(key)
    ...
    for slice_num in range(config.DATA.SLICE_NUMBER):
        ...
        for key, value in t_model.state_dict().items():
            logger.info(key)
        # print(t_model.state_dict())
        # print(s_model.state_dict())
        for key, value in t_model.state_dict().items():
            if key in s_model.state_dict().keys():
                new_teacher_dict[key] = (
                        s_model[key] * (1 - keep_rate) + value * keep_rate)
            else:
                raise Exception("{} is not found in student model".format(key))
        t_model.load_state_dict(new_teacher_dict)
...
if __name__ == "__main__":
    ...
    for i in range(num):
        ...
        if i == num-1:
            ...
            logger.info(f"plcc1 improve:{plcc1_train_average - plcc1_notrain_average};"
                        f"srcc1 improve:{srcc1_train_average - srcc1_notrain_average};"
                        f"plcc2 improve:{plcc2_train_average - plcc2_notrain_average};"
                        f"srcc2 improve:{srcc2_train_average - srcc2_notrain_average}")
