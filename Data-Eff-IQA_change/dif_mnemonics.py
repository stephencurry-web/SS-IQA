...
from examplar import get_A_examplar, get_B_examplar, get_mnemonics
...
def main(config):
    ...
    # # dataset A 训练集以及测试集
    dataset_train_A, dataset_val_A, data_loader_train_A, data_loader_train_val_A, data_loader_val_A, mixup_fn_A = IQA_build_loader_a_krr(config)
    logger.info(f"{len(data_loader_train_val_A)}")
    # dataset B 训练集以及测试集
    dataset_train_B, dataset_val_B, data_loader_train_B, data_loader_train_val_B, data_loader_val_B, mixup_fn_B = IQA_build_loader_b_krr(config)
    ...
    # x = []
    # plcc_graph = [[]for i in range(4)]
    # srcc_graph = [[]for i in range(4)]
    # loss_graph = [[]for i in range(4)]
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        # x.append(epoch)
        srcc_1_a, plcc_1_a, loss_1_a = train_one_epoch(config, t_model, criterion, data_loader_train_A,
                        optimizer_A, epoch, mixup_fn_A, lr_scheduler_A, loss_scaler1, writer, wandb_runner)

        # plcc_graph[0].append(plcc_1_a)
        # srcc_graph[0].append(srcc_1_a)
        # loss_graph[0].append(loss_1_a)

        # dataset A测试
        srcc_2_a, plcc_2_a, loss_2_a = validate(config, data_loader_val_A, t_model, epoch, 1,
                                                len(dataset_val_A), writer, wandb_runner)
        # plcc_graph[1].append(plcc_2_a)
        # srcc_graph[1].append(srcc_2_a)
        # loss_graph[1].append(loss_2_a)
        ...
        # srcc_1_b, plcc_1_b, loss_1_b = validate(config, data_loader_train_val_B, t_model, epoch, 2,
        #                                         len(dataset_train_B), writer, wandb_runner)
        # plcc_graph[2].append(plcc_1_b)
        # srcc_graph[2].append(srcc_1_b)
        # loss_graph[2].append(loss_1_b)

        # dataset B测试
        srcc_2_b, plcc_2_b, loss_2_b = validate(config, data_loader_val_B, t_model, epoch, 3,
                                                 len(dataset_val_B), writer, wandb_runner)
        # plcc_graph[3].append(plcc_2_b)
        # srcc_graph[3].append(srcc_2_b)
        # loss_graph[3].append(loss_2_b)
    ...
    # examplar_total = get_A_examplar(examplar_total, dataset_train_A, data_loader_train_ex1, s_model, config.DATA.EXAMPLAR_STEP, logger)
    examplar_total = get_mnemonics(config, examplar_total, dataset_train_A, s_model, config.DATA.EXAMPLAR_STEP, criterion, config.DATA.BATCH_SIZE, logger)
    if dist.get_rank() == 0:
    ...
    for slice_num in range(config.DATA.SLICE_NUMBER):
        ...
        dataset_train1 = build_IQA_dataset_krr(config, train_index)
        pseudo_label_data = get_label(s_model, data_loader_train_ex1, len(dataset_train_A),
                                      dataset_train1, config.DATA.BATCH_SIZE1, logger)
        data_loader_train1, mixup_fn = IQA_build_loader_knn(config, pseudo_label_data, examplar_total)
        ...
        optimizer_B = build_optimizer1(config, s_model)
        if config.TRAIN.ACCUMULATION_STEPS > 1:
        ...
        for epoch in range(config.TRAIN.START_EPOCH_STU1, config.TRAIN.EPOCHS_STU1):
            ...
            # x.append(config.TRAIN.EPOCHS + slice_num * config.TRAIN.EPOCHS_STU1 + epoch)
            # x.append(epoch + 1)

            srcc_1_b, plcc_1_b, loss_1_b = train_one_epoch2(config, s_model, criterion, data_loader_train1, slice_num,
            ...
            #
            # srcc_1_a, plcc_1_a, loss_1_a = validate2(config, data_loader_train_val_A, s_model, epoch, slice_num, 1,
            #                                          len(dataset_train_A), writer, wandb_runner)
            # plcc_graph[0].append(plcc_1_a)
            # srcc_graph[0].append(srcc_1_a)
            # loss_graph[0].append(loss_1_a)

            # dataset A测试
            srcc_2_a, plcc_2_a, loss_2_a = validate2(config, data_loader_val_A, s_model, epoch, slice_num, 2,
                                                  len(dataset_val_A), writer, wandb_runner)
            ...
            # dataset B测试
            srcc_2_b, plcc_2_b, loss_2_b = validate2(config, data_loader_val_B, s_model, epoch, slice_num, 3,
                                                     len(dataset_val_B), writer, wandb_runner)
            # plcc_graph[3].append(plcc_2_b)
            # srcc_graph[3].append(srcc_2_b)
            # loss_graph[3].append(loss_2_b)
            ...
            # _update_teacher_model(t_model, s_model)
        ...
        if slice_num < config.DATA.SLICE_NUMBER-1:
            # for select examplar
            dataset_train2 = build_IQA_dataset_krr(config, trained_index)
            pseudo_label_data_examplar = get_label(s_model, data_loader_train_ex1, len(dataset_train_A),
                                          dataset_train2, config.DATA.BATCH_SIZE1, logger)
            data_loader_train_ex2 = torch.utils.data.DataLoader(
                pseudo_label_data_examplar,
            ...
            # examplar_total = get_A_examplar(examplar_total, dataset_train_A, data_loader_train_ex1, s_model, config.DATA.EXAMPLAR_STEP, logger)
            examplar_total = get_mnemonics(config, examplar_total, dataset_train_A, s_model,
                                               config.DATA.EXAMPLAR_STEP, criterion, config.DATA.BATCH_SIZE, logger)
            # select data B examplar
            # examplar_total = get_A_examplar(examplar_total, pseudo_label_data_examplar, data_loader_train_ex2, s_model, config.DATA.EXAMPLAR_STEP1, logger)
            examplar_total = get_mnemonics(config, examplar_total, pseudo_label_data_examplar, s_model,
                                               config.DATA.EXAMPLAR_STEP1, criterion, config.DATA.BATCH_SIZE1, logger)
            if dist.get_rank() == 0:
                logger.info(f"len(examplar_total):{len(examplar_total)}")
    ...
    # if dist.get_rank() == 0:
    #     wandb.log({"plcc": wandb.plot.line_series(
    #         xs=x,
    #         ys=[plcc_graph[0], plcc_graph[1], plcc_graph[2], plcc_graph[3]],
    #         keys=["A_train", "A_test", "B_train", "B_test"],
    #         title="plcc",
    #         xname="epoch",
    #         )
    #     })
    #     wandb.log({"srcc": wandb.plot.line_series(
    #         xs=x,
    #         ys=[srcc_graph[0], srcc_graph[1], srcc_graph[2], srcc_graph[3]],
    #         keys=["A_train", "A_test", "B_train", "B_test"],
    #         title="srcc",
    #         xname="epoch",
    #     )
    ...
    #         xs=x,
    #         ys=[loss_graph[0], loss_graph[1], loss_graph[2], loss_graph[3]],
    #         keys=["A_train", "A_test", "B_train", "B_test"],
    #         title="loss",
    #         xname="epoch",
    #     )
...
def train_one_epoch2(
    ...
    for idx, (samples, _, targets, data_num) in enumerate(data_loader):
        samples = samples.cuda(non_blocking=True)

        # with torch.no_grad():
            # targets, feat = t_model(samples)
    ...
    return train_srcc, train_plcc, loss_meter.avg
...
def train_one_epoch(
    ...
    for idx, (samples, _, targets, _) in enumerate(data_loader):
        samples = samples.cuda(non_blocking=True)
        targets = targets.cuda(non_blocking=True)
        if mixup_fn is not None:
            samples, targets = mixup_fn(samples, targets)
