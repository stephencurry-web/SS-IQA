...
from models import build_model, build_model_pws, get_label, get_pseudo_label
...
def main(config):
    ...
    for slice_num in range(config.DATA.SLICE_NUMBER):
        ...
        pseudo_label_data = get_pseudo_label(s_model, dataset_train1, config.DATA.BATCH_SIZE1, logger)
        ...
        for epoch in range(config.TRAIN.START_EPOCH_STU1, config.TRAIN.EPOCHS_STU1):
            ...
            srcc_1_b, plcc_1_b, loss_1_b = train_one_epoch2(config, s_model, criterion, data_loader_train1, slice_num,
                                                            optimizer_B, epoch, mixup_fn, lr_scheduler_B, loss_scaler2,
                                                            writer, wandb_runner)
        ...
        if slice_num < config.DATA.SLICE_NUMBER-1:
            ...
            pseudo_label_data_examplar = get_pseudo_label(s_model, dataset_train2, config.DATA.BATCH_SIZE1, logger)
