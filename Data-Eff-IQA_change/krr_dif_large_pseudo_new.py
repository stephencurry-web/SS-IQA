...
from models import build_model, build_model_pws1, get_krr_label, get_regressor, get_pseudo_label
...
def main(config, save_num):
    ...
    for slice_num in range(config.DATA.SLICE_NUMBER):
        ...
        pseudo_label = get_pseudo_label(config, t_model, dataset_train1_teacher, config.DATA.BATCH_SIZE1, logger)
        ...
        if slice_num < config.DATA.SLICE_NUMBER-1:
            ...
            for i in range(slice_num+1):
                ...
                pseudo_label_examplar = get_pseudo_label(config, t_model, dataset_train2_teacher, config.DATA.BATCH_SIZE1, logger)
