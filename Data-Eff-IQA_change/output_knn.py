...
from IQA import (
    IQA_build_loader, IQA_build_loader2,
    IQA_build_loader3, IQA_build_loader5,
    IQA_build_dataset_knn, IQA_build_loader_knn
)
...
from examplar import get_A_examplar, get_B_examplar
...
def main(config):
    ...
    # dataset A 训练集以及测试集
    dataset_train_A, dataset_val_A, data_loader_train_A, data_loader_train_val_A, data_loader_val_A, mixup_fn_A = IQA_build_loader(config, 1)
    logger.info(f"{len(data_loader_train_val_A)}")
    # dataset B 训练集以及测试集
    dataset_train_B, dataset_val_B, data_loader_train_B, data_loader_train_val_B, data_loader_val_B, mixup_fn_B = IQA_build_loader2(config, 0)
    ...
    for slice_num in range(config.DATA.SLICE_NUMBER):
        ...
        # dataset_train1, data_loader_train1, mixup_fn = IQA_build_loader3(config, train_index, examplar_total, 0)

        dataset_train1 = IQA_build_dataset_knn(config, train_index, 0)
        pseudo_label_data = get_label(t_model, data_loader_train_ex1, len(dataset_train_A),
                                      dataset_train1, config.DATA.BATCH_SIZE1)
        ...
        if slice_num < config.DATA.SLICE_NUMBER-1:
            # for select examplar
            dataset_train2 = IQA_build_dataset_knn(config, trained_index, 0)
            pseudo_label_data_examplar = get_label(t_model, data_loader_train_ex1, len(dataset_train_A),
                                          dataset_train2, config.DATA.BATCH_SIZE1)
            ...
            examplar_total = get_A_examplar(examplar_total, pseudo_label_data_examplar, data_loader_train_ex2, s_model, config.DATA.EXAMPLAR_STEP1)
            if dist.get_rank() == 0:
                logger.info(f"len(examplar_total):{len(examplar_total)}")
...
def train_one_epoch2(
    ...
    for idx, (samples, targets, data_num) in enumerate(data_loader):
