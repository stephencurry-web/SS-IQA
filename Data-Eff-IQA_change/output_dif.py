...
from IQA import IQA_build_loader_dif, IQA_build_loader2_dif, IQA_build_loader3_dif, IQA_build_loader5_dif
...
def main(config):
    ...
    for slice_num in range(config.DATA.SLICE_NUMBER):
        ...
        dataset_train1, data_loader_train1, mixup_fn = IQA_build_loader3_dif(config, train_index, examplar_total, 0)
        if dist.get_rank() == 0:
        ...
        if slice_num < config.DATA.SLICE_NUMBER-1:
            # for select examplar
            dataset_train2 = IQA_build_loader5_dif(config, train_index, train_set, 0)
