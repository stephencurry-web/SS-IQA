...
class KONIQDATASET4(data.Dataset):
    def __init__(self, root, index, patch_num, examplar, data_num, transform=None, transform1=None):
        super(KONIQDATASET4, self).__init__()
        ...
        for _, item in enumerate(index):
            for _ in range(patch_num):
                ...
                one_sample1 = transform(one_sample)
                one_sample2 = transform1(one_sample)
                sample.append((one_sample1, one_sample2, label, data_num))
        # if dist.get_rank() == 0:
        #     print("sample[-1]")
        #     print(sample[-1])
    ...
    def __getitem__(self, index):
        ...
        """
        sample1, sample2, target, data_num = self.samples[index]
...
class LIVECDATASET4(data.Dataset):
    def __init__(self, root, index, patch_num, examplar, data_num, transform=None, transform1=None):
...
class LIVEDataset4(data.Dataset):
    def __init__(self, root, index, patch_num, examplar, data_num, transform=None, transform1=None):
        ...
        for i in range(0, len(index)):
            ...
            for j, item in enumerate(train_sel):
                for aug in range(patch_num):
                    ...
                    one_sample1 = transform(one_sample)
                    one_sample2 = transform1(one_sample)
                    sample.append((one_sample1, one_sample2, transfer("live", labels[0][item]), data_num))
            # if dist.get_rank() == 0:
            #     print("sample[-1]")
            #     print(sample[-1])
        for i, item in enumerate(examplar):
...
class TID2013Dataset4(data.Dataset):
...
class CSIQDataset4(data.Dataset):
    def __init__(self, root, index, patch_num, examplar, data_num, transform=None, transform1=None):
...
class KADIDDataset4(data.Dataset):
    def __init__(self, root, index, patch_num, examplar, data_num, transform=None, transform1=None):
        refpath = os.path.join(root, "reference_images")
...
class SPAQDATASET4(data.Dataset):
    def __init__(self, root, index, patch_num, examplar, data_num, transform=None, transform1=None):
        super(SPAQDATASET4, self).__init__()
...
class FBLIVEFolder4(data.Dataset):
    def __init__(self, root, index, patch_num, examplar, data_num, transform=None, transform1=None):
        imgname = []
        mos_all = []
