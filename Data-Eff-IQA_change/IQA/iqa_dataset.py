...
class KONIQDATASET(data.Dataset):
    def __init__(self, root, index, patch_num, data_num,  transform=None):
    ...
    def __getitem__(self, index):
        ...
        path, target, data_num = self.samples[index]
    ...
    def __init__(self, root, index, patch_num, data_num, transform=None):
        ...
        for i, item in enumerate(index):
            for aug in range(patch_num):
                sample.append(
                    (os.path.join(root, "Images", imgpath[item][0][0]), labels[item], data_num)
                )
    ...
    def __init__(self, root, index, patch_num, data_num, transform=None):
        ...
        for i in range(0, len(index)):
            ...
            for j, item in enumerate(train_sel):
                for aug in range(patch_num):
                    sample.append((imgpath[item], labels[0][item], data_num))
        self.samples = sample
        self.transform = transform
...
class FBLIVEFolder(data.Dataset):
    def __init__(self, root, index, patch_num, data_num,  transform=None):
        ...
        for i, item in enumerate(index):
            for aug in range(patch_num):
                sample.append(
                    (os.path.join(root, "database", imgname[item]), mos_all[item], data_num)
                )
