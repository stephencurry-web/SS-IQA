...
from models import build_model, build_model_pws1, get_krr_label, get_pseudo_label
...
def main(config, i):
    ...
    for slice_num in range(config.DATA.SLICE_NUMBER):
        ...
        pseudo_label = get_pseudo_label(t_model, dataset_train1_teacher, config.DATA.BATCH_SIZE1, logger)
        ...
        if dist.get_rank() == 0:
            logger.info(f"len(dataset_train1):{len(dataset_train1_teacher)}")
            logger.info(f"len(dataset_train1):{len(dataset_train1_student)}")
        ...
        if slice_num < config.DATA.SLICE_NUMBER-1:
            ...
            for i in range(slice_num+1):
                ...
                pseudo_label_examplar = get_pseudo_label(t_model, dataset_train2_teacher, config.DATA.BATCH_SIZE1, logger)
