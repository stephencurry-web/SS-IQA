...
def main(config):
    ...
    # examplar_total = get_best_examplar(config, examplar_total, dataset_train_A, s_model, config.DATA.EXAMPLAR_STEP, criterion, config.DATA.BATCH_SIZE)
    if dist.get_rank() == 0:
    ...
    for slice_num in range(config.DATA.SLICE_NUMBER):
        ...
        if config.TRAIN.ACCUMULATION_STEPS > 1:
            lr_scheduler_B = build_scheduler2(
                config, optimizer_B, len(data_loader_train1) // config.TRAIN.ACCUMULATION_STEPS
            )
        else:
            lr_scheduler_B = build_scheduler2(config, optimizer_B, len(data_loader_train1))
        ...
        if slice_num < config.DATA.SLICE_NUMBER-1:
            ...
            examplar_total = get_A_examplar(examplar_total, dataset_train_A, data_loader_train_ex1, s_model, config.DATA.EXAMPLAR_STEP, logger)
            # examplar_total = get_best_examplar(config, examplar_total, dataset_train_A, s_model,
            ...
            # examplar_total = get_best_examplar(config, examplar_total, pseudo_label_data_examplar, s_model,
