...
from IQA import IQA_build_loader, IQA_build_loader2, IQA_build_loader3, IQA_build_loader5
from config import get_config
from logger import create_logger
from lr_scheduler import build_scheduler1, build_scheduler2
...
def main(config):
    ...
    # dataset A 训练集以及测试集
    dataset_train_A, dataset_val_A, data_loader_train_A, data_loader_val_A, mixup_fn_A = IQA_build_loader(config, 1)
    # dataset B 训练集以及测试集
    dataset_train_B, dataset_val_B, data_loader_train_B, data_loader_val_B, mixup_fn_B = IQA_build_loader2(config, 0)
    # 打印
    if dist.get_rank() == 0:
    ...
    # s_model的optimizer和lr_scheduler
    optimizer_A = build_optimizer(config, s_model)
    ...
    optimizer_T = build_optimizer(config, t_model)
    if config.TRAIN.ACCUMULATION_STEPS > 1:
        lr_scheduler_T = build_scheduler1(
            config, optimizer_T, len(data_loader_train_A) // config.TRAIN.ACCUMULATION_STEPS
        )
    else:
        lr_scheduler_T = build_scheduler1(config, optimizer_T, len(data_loader_train_A))
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        train_one_epoch_teacher(config, t_model, criterion, data_loader_train_A,
                        optimizer_T, epoch, mixup_fn_A, lr_scheduler_T, loss_scaler3, writer, wandb_runner)
        # dataset A测试
        srcc_1_a, plcc_1_a, loss_1_a = validate(config, data_loader_val_A, t_model, epoch, 1,
        ...
        logger.info(
            f"PLCC and SRCC of the network on the {len(dataset_val_A)} test images: {plcc_1_a:.6f}, {srcc_1_a:.6f}"
        )
        ...
        # dataset B测试
        srcc_1_b, plcc_1_b, loss_1_b = validate1(config, data_loader_val_B, t_model, epoch, 2,
        ...
        logger.info(
            f"PLCC and SRCC of the network on the {len(dataset_val_B)} test images: {plcc_1_b:.6f}, {srcc_1_b:.6f}"
        )
    ...
    # 在 dataset A上训练student
    for epoch in range(config.TRAIN.START_EPOCH_STU, config.TRAIN.EPOCHS_STU):
        data_loader_train_A.sampler.set_epoch(epoch)
        
        train_one_epoch(config, s_model, criterion, data_loader_train_A,
            optimizer_A, epoch, mixup_fn_A, lr_scheduler_A, loss_scaler1, writer, wandb_runner)
        # dataset A测试
        srcc_1_a, plcc_1_a, loss_1_a = validate(config, data_loader_val_A, s_model, epoch, 1,
                                                                     len(dataset_val_A), writer, wandb_runner)
        if dist.get_rank() == 0:
            wandb_runner.log(
                {"Validating srcc1": srcc_1_a, "Validating plcc1": plcc_1_a, "Epoch": epoch}
        ...
        # dataset B测试
        srcc_1_b, plcc_1_b, loss_1_b = validate1(config, data_loader_val_B, s_model, epoch, 2,
                                                               len(dataset_val_B), writer, wandb_runner)
        if dist.get_rank() == 0:
            wandb_runner.log(
                {"Validating srcc2": srcc_1_b, "Validating plcc2": plcc_1_b, "Epoch": epoch}
    ...
    # 取dataset A的examplar
    # for select exampar,batch_size设1
    data_loader_train_ex1 = torch.utils.data.DataLoader(
        dataset_train_A,
        batch_size=1,
    )
    examplar_total = get_A_examplar(examplar_total, dataset_train_A, data_loader_train_ex1, s_model, criterion, config.DATA.EXAMPLAR_STEP)
    ...
    for slice_num in range(config.DATA.SLICE_NUMBER):
        ...
        dataset_train1, dataset_val1, data_loader_train1, data_loader_val1, mixup_fn \
                = IQA_build_loader3(config, train_index, config.SET1.TEST_INDEX, examplar_total, 0)
        if dist.get_rank() == 0:
        ...
        for epoch in range(config.TRAIN.START_EPOCH_STU1, config.TRAIN.EPOCHS_STU1):
            ...
            # dataset A测试
            srcc_2_a, plcc_2_a, loss_2_a = validate2(config, data_loader_val_A, s_model, epoch, slice_num, 1,
                                                  len(dataset_val_A), writer, wandb_runner)
            # dataset B测试
            srcc_2_b, plcc_2_b, loss_2_b = validate3(config, data_loader_val_B, s_model, epoch, slice_num, 2,
                                                     len(dataset_val_B), writer, wandb_runner)
        ...
        # for select examplar
        dataset_train2, _, _, _, mixup_fn = IQA_build_loader5(config, train_index, config.SET1.TEST_INDEX, train_set, 0)
        ...
        if slice_num < config.DATA.SLICE_NUMBER-1:
            ...
            examplar_total = get_B_examplar(examplar_total, dataset_train2, data_loader_train2_ex2, s_model, t_model, criterion, config.DATA.EXAMPLAR_STEP1)
        if dist.get_rank() == 0:
            logger.info(f"len(examplar_total):{len(examplar_total)}")
        # EMA更新teacher
        for key in s_model.state_dict().keys():
            logger.info(key)
        for key, value in t_model.state_dict().items():
            logger.info(key)
